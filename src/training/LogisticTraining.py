...
from src.data.dataset import LogisticRegressionDataset
...
def get_data_loaders():
    ...
    dev_features = torch.tensor(dev_features.toarray(), dtype=torch.float32)
    test_features = torch.tensor(test_features.toarray(), dtype=torch.float32)

    train_labels = torch.tensor(train_set['label'].tolist(), dtype=torch.float32)
    dev_labels = torch.tensor(dev_set['label'].tolist(), dtype=torch.float32)
    test_labels = torch.tensor(test_set['label'].tolist(), dtype=torch.float32)

    train_dataset = LogisticRegressionDataset(train_features, train_labels)
    dev_dataset = LogisticRegressionDataset(dev_features, dev_labels)
    test_dataset = LogisticRegressionDataset(test_features, test_labels)
